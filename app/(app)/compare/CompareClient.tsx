...
type CompareClientProps = {
  initialMarketUniverse?: Stock[];
};

export default function ComparePage({ initialMarketUniverse = [] }: CompareClientProps) {
  ...
  const [marketUniverse, setMarketUniverse] = useState<Stock[]>(() => initialMarketUniverse.filter((stock) => stock.assetType !== "INDEX"));
  const [universeLoading, setUniverseLoading] = useState(!initialMarketUniverse.length);
  ...
  useEffect(() => {
    ...
    fetch("/api/stocks/universe?limit=160")
      .then((res) => {
        if (!res.ok) throw new Error("Unable to load market universe");
        return res.json();
      })
      .then((payload) => {
        if (!cancelled) setMarketUniverse((payload.stocks || []).filter((stock: Stock) => stock.assetType !== "INDEX"));
      })
      .catch(() => {
        if (!cancelled) {
          setMarketUniverse(STOCKS.filter((stock) => stock.assetType !== "INDEX"));
        }
      })
      .finally(() => {
  ...
  useEffect(() => {
    try {
      ...
      /* dynamic defaults will be selected from market data */
    }
  }, []);
  ...
  const rankedUniverse = useMemo(() => {
    return marketUniverse
      .filter((stock) => stock.assetType !== "INDEX" && Number.isFinite(stock.price) && stock.price > 0)
      .map((stock) => ({ stock, val: computeValuation(stock, defaultDCFParams(stock)) }))
      .sort((a, b) => b.val.marginOfSafety - a.val.marginOfSafety);
  }, [marketUniverse]);

  const symbolsFrom = (items: typeof rankedUniverse, max = 4) => {
  ...
  const dynamicDefaultSymbols = useMemo(() => {
    const thai = rankedUniverse.find((item) => item.stock.assetType === "TH_STOCK");
    const nasdaq = rankedUniverse.find((item) => item.stock.assetType === "US_STOCK" && item.stock.market === "NASDAQ");
    const bitcoin = rankedUniverse.find((item) => item.stock.symbol.toUpperCase() === "BTC") || rankedUniverse.find((item) => item.stock.assetType === "CRYPTO");
    const gold = rankedUniverse.find((item) => item.stock.symbol.toUpperCase() === "GOLD") || rankedUniverse.find((item) => item.stock.assetType === "FUTURES");
    return symbolsFrom([thai, nasdaq, bitcoin, gold, ...rankedUniverse].filter(Boolean) as typeof rankedUniverse, 4);
  }, [rankedUniverse]);

  const comparePresets = useMemo(() => {
    const thai = symbolsFrom(rankedUniverse.filter((item) => item.stock.assetType === "TH_STOCK"), 4);
    const nyse = symbolsFrom(rankedUniverse.filter((item) => item.stock.assetType === "US_STOCK" && item.stock.market === "NYSE"), 4);
    const nasdaq = symbolsFrom(rankedUniverse.filter((item) => item.stock.assetType === "US_STOCK" && item.stock.market === "NASDAQ"), 4);
    const crypto = symbolsFrom(rankedUniverse.filter((item) => item.stock.assetType === "CRYPTO"), 4);
    const futures = symbolsFrom(rankedUniverse.filter((item) => item.stock.assetType === "FUTURES"), 4);
    const bitcoinGold = symbolsFrom(
      [
        rankedUniverse.find((item) => item.stock.symbol.toUpperCase() === "BTC"),
        rankedUniverse.find((item) => item.stock.symbol.toUpperCase() === "GOLD"),
        ...rankedUniverse.filter((item) => item.stock.assetType === "CRYPTO" || item.stock.assetType === "FUTURES"),
      ].filter(Boolean) as typeof rankedUniverse,
      4
    );
    const dividend = symbolsFrom(
      [...rankedUniverse]
        .filter((item) => Number.isFinite(item.val.ratios.dividendYield) && item.val.ratios.dividendYield > 0)
        .sort((a, b) => b.val.ratios.dividendYield - a.val.ratios.dividendYield),
      4
    );
    const value = symbolsFrom(rankedUniverse.filter((item) => item.val.marginOfSafety > 0), 4);
    const multiMarket = symbolsFrom(
      [
        rankedUniverse.find((item) => item.stock.assetType === "TH_STOCK"),
        rankedUniverse.find((item) => item.stock.assetType === "US_STOCK" && item.stock.market === "NASDAQ"),
        rankedUniverse.find((item) => item.stock.symbol.toUpperCase() === "BTC"),
        rankedUniverse.find((item) => item.stock.symbol.toUpperCase() === "GOLD"),
        ...rankedUniverse,
      ].filter(Boolean) as typeof rankedUniverse,
    ...
    return [
      { id: "multi-market", symbols: multiMarket, labelTh: "หลายตลาด", labelEn: "Multi-market" },
      { id: "thai", symbols: thai, labelTh: "ตลาดไทย", labelEn: "Thai Market" },
      { id: "nyse", symbols: nyse, labelTh: "NYSE", labelEn: "NYSE" },
      { id: "nasdaq", symbols: nasdaq, labelTh: "Nasdaq", labelEn: "Nasdaq" },
      { id: "bitcoin-gold", symbols: bitcoinGold, labelTh: "Bitcoin / ทองคำ", labelEn: "Bitcoin / Gold" },
      { id: "crypto", symbols: crypto, labelTh: "คริปโต", labelEn: "Crypto" },
      { id: "futures", symbols: futures, labelTh: "ทองคำ / สินค้าโภคภัณฑ์", labelEn: "Gold / Commodities" },
      { id: "value", symbols: value, labelTh: "MOS สูง", labelEn: "High MOS" },
      { id: "dividend", symbols: dividend, labelTh: "ปันผลเด่น", labelEn: "Dividend" },
    ].filter((preset) => preset.symbols.length >= 2);
  }, [rankedUniverse]);

  useEffect(() => {
    if (picked.length >= 2 || universeLoading || dynamicDefaultSymbols.length < 2) return;
    setPicked(dynamicDefaultSymbols);
    setSetName(dynamicDefaultSymbols.join(" vs "));
  }, [dynamicDefaultSymbols, picked.length, universeLoading]);
  ...
  const applyPreset = (symbols: string[]) => {
    const next = Array.from(
      new Set(
        symbols
          .map((symbol) => symbol.trim().toUpperCase())
          .filter((symbol) => stockBySymbol.has(symbol) || /^[A-Z0-9.-]{1,20}$/.test(symbol))
      )
    ).slice(0, 4);
    if (next.length >= 2) {
      setPicked(next);
  ...
  const applyWatchlistPreset = () => {
    const next = watchlist.map((symbol) => symbol.toUpperCase()).filter((symbol) => stockBySymbol.has(symbol)).slice(0, 4);
    if (next.length < 2) {
  ...
  }, [marketUniverse, picked, searchQuery]);
  ...
  const stocks = picked.map((sym) => stockBySymbol.get(sym.toUpperCase())).filter(Boolean) as Stock[];
  ...
  const add = (sym: string) => {
    ...
    if (picked.length < 4 && normalized && !picked.includes(normalized)) {
      setPicked((p) => [...p, normalized]);
      setSearchQuery("");
    }
  };
  ...
  const getWinnerSymbol = (metric: string) => {
    ...
    const scores = picked.map((sym) => {
      const s = stockBySymbol.get(sym.toUpperCase());
      if (!s) return { sym, score: -99999 };
        ...
        <div className="flex flex-col gap-3 lg:flex-row lg:items-center lg:justify-between">
          <div>
            ...
            <p className="mt-1 text-[11px] font-semibold leading-relaxed text-muted">
              {lang === "th"
                ? "เลือกชุดเปรียบเทียบจากหลายตลาด หรือใช้ Watchlist ของคุณ โดยคำนวณจากข้อมูลล่าสุดในระบบ"
                : "Choose dynamic comparison sets across markets or load your watchlist using the latest available market data."}
            </p>
          </div>
          <div className="flex flex-wrap gap-2">
            ...
            <button
              type="button"
              onClick={() => applyPreset(dynamicDefaultSymbols)}
              disabled={dynamicDefaultSymbols.length < 2}
            ...
            >
              {universeLoading ? (lang === "th" ? "กำลังโหลด" : "Loading") : (lang === "th" ? "รีเซ็ตหลายตลาด" : "Reset Markets")}
            </button>
          </div>
        </div>
      </Card>
            ...
            <div className="mt-4 grid gap-2 sm:grid-cols-[minmax(0,1fr)_auto]">
              ...
              <div className="grid gap-2 sm:grid-cols-2">
                ...
                <Button
                  ...
                  disabled={telegramSending || picked.length < 2 || !user?.email}
                  className="w-full"
                ...
                savedSets.map((set) => (
                  <div key={set.id} className="flex items-center gap-2 rounded-lg border border-line bg-surface/60 p-2">
                    ...
                    <button
                      ...
                      disabled={telegramSending || !user?.email}
