...
function eventTitle(event: CalendarEventRow) {
  return event.name || event.holidayName || event.exchange || "-";
}

function eventDetailRows(event: CalendarEventRow, type: CalendarType, lang: "th" | "en") {
  const rows: Array<{ label: string; value?: string | number | null; tone?: "gold" | "brand" }> = [
    { label: lang === "th" ? "วันที่" : "Date", value: formatDate(event.date, lang) },
    { label: lang === "th" ? "เวลา" : "Time", value: event.time || "-" },
    { label: lang === "th" ? "ประเทศ/ตลาด" : "Country/Market", value: countryLabel(event) },
    { label: lang === "th" ? "สกุลเงิน" : "Currency", value: event.currency || event.exchange || "-" },
    { label: lang === "th" ? "ความสำคัญ" : "Impact", value: impactLabel(event.importance, lang), tone: "gold" },
  ];
  ...
  if (type === "economic") {
    ...
    rows.push(
      { label: "EPS", value: event.eps || "-" },
      { label: "EPS Forecast", value: event.epsForecast || "-" },
      { label: "Revenue", value: event.revenue || "-" },
      { label: "Revenue Forecast", value: event.revenueForecast || "-" }
    );
  } else if (type === "dividends") {
    rows.push(
      { label: lang === "th" ? "เงินปันผล" : "Dividend", value: event.dividend || "-" },
      { label: "Yield", value: event.dividendYield || "-" }
    );
  } else if (type === "stock_split") {
    rows.push({ label: lang === "th" ? "อัตราแตกหุ้น" : "Split Ratio", value: event.splitRatio || "-" });
  } else if (type === "ipo") {
    rows.push(
      { label: "IPO Price", value: event.ipoPrice || "-" },
      { label: "IPO Value", value: event.ipoValue || "-" }
    );
  } else if (type === "holiday") {
    rows.push({ label: lang === "th" ? "ชื่อวันหยุด" : "Holiday", value: event.holidayName || event.name || "-" });
  }
...
export default function EconomicCalendarPage() {
  ...
  }, [activeType, timeFilter, minImportance, user?.email, canAccess]);
    ...
    <div className="mx-auto max-w-6xl space-y-6">
      <section className="rounded-2xl border border-line bg-surface p-5 shadow-card">
        <div className="flex flex-col gap-4 lg:flex-row lg:items-end lg:justify-between">
          ...
          <Button variant="outline" onClick={loadCalendar} disabled={loading || !user?.email}>
...
function CalendarEventLine({
  ...
  return (
    <button
      ...
      className="grid w-full gap-2 px-3 py-3 text-left transition hover:bg-elevate/35 focus:outline-none focus:ring-2 focus:ring-brand/40 lg:grid-cols-[112px_172px_minmax(0,1fr)_150px_190px] lg:items-center lg:gap-3"
    >
...
function CalendarEventModal({
  ...
  const flag = countryFlag(event.country, event.currency);
  const meta = eventMeta(event, activeType);
  const detailRows = eventDetailRows(event, activeType, lang);

  return (
    <div className="fixed inset-0 z-50 grid place-items-center bg-bg/80 px-4 py-6 backdrop-blur-sm" role="dialog" aria-modal="true">
      <div className="max-h-[88vh] w-full max-w-2xl overflow-hidden rounded-2xl border border-line bg-surface shadow-card">
        <div className="flex items-start justify-between gap-4 border-b border-line p-4 sm:p-5">
          <div className="min-w-0">
            <div className="flex items-center gap-2 text-[11px] font-black uppercase tracking-wide text-muted">
              <span className="text-lg">{flag}</span>
              <span>{countryLabel(event)}</span>
            ...
            <p className="mt-1 text-xs font-semibold leading-relaxed text-muted">
              {lang === "th"
                ? "รายละเอียดนี้แสดงภายใน ValuStock เพื่อให้ติดตามเหตุการณ์ได้โดยไม่ต้องออกจากหน้าเว็บ"
                : "This detail view is shown inside ValuStock so you can review the event without leaving the page."}
            </p>
          </div>
          <button
            type="button"
