"use client";

import { useState, useEffect } from "react";
import Link from "next/link";
import { STOCKS } from "@/lib/stocks";
import { AssetLogo } from "@/components/AssetLogo";
import { Card } from "@/components/ui/Card";
import { Button } from "@/components/ui/Button";
import { computeValuation, defaultDCFParams } from "@/lib/valuation";
import { useTranslation } from "@/lib/translations";
import { baht, dollar, pct, num } from "@/lib/format";
import { Sparkles, Target, Info, Shield } from "@/lib/icons";

export default function UndervaluedStocksPage() {
  const { lang, t } = useTranslation();
  const [openFaq, setOpenFaq] = useState<number | null>(null);

  // Filter top undervalued stocks (MOS >= 15%)
  const list = STOCKS.map((s: any) => ({
    stock: s,
    val: computeValuation(s, defaultDCFParams(s)),
  }))
  .filter((x: any) => x.val.verdict === "undervalued" && x.val.marginOfSafety >= 15)
  .sort((a: any, b: any) => b.val.marginOfSafety - a.val.marginOfSafety)
  .slice(0, 10); // Display top 10

  const topThree = list.slice(0, 3).map(x => x.stock.symbol).join(", ");

  const titleTH = "Best Undervalued Stocks: สแกนหุ้นราคาถูกกว่ามูลค่าจริง มีส่วนต่าง MOS สูง";
  const titleEN = "Best Undervalued Stocks: Real-Time Intrinsic Value & Margin of Safety Targets";
  const descTH = "คัดหุ้นเด่นทั้งตลาดไทยและตลาดสหรัฐฯ ที่ราคาตลาดลดกระหน่ำต่ำกว่ามูลค่าที่แท้จริงไม่น้อยกว่า 15% พร้อมเครื่องประเมินความเสี่ยงและโครงสร้างงบเพื่อนักลงทุนคุณค่า";
  const descEN = "Discover the top undervalued equities trading at deep pricing discounts relative to computed intrinsic targets. Ideal for long-term margin-of-safety compounders.";

  useEffect(() => {
    document.title = `${lang === "th" ? titleTH : titleEN} | ValuStock`;
    let metaDesc = document.querySelector('meta[name="description"]');
    if (!metaDesc) {
      metaDesc = document.createElement('meta');
      metaDesc.setAttribute('name', 'description');
      document.head.appendChild(metaDesc);
    }
    metaDesc.setAttribute('content', lang === "th" ? descTH : descEN);
  }, [lang]);

  const faqs = [
    {
      q: lang === "th" ? "หุ้น Undervalued คืออะไร และคัดสรรอย่างไร?" : "What is an undervalued stock and how do we screen them?",
      a: lang === "th"
        ? "หุ้น Undervalued คือหุ้นที่มีราคาซื้อขายปัจจุบันต่ำกว่ามูลค่าที่แท้จริง (Fair Value) ซึ่งวิเคราะห์มาจากแบบจำลองการเงินกระแสเงินสดคิดลด (DCF) โดยระบบจะคัดสรรเฉพาะกิจการที่มีส่วนต่างความมั่นคง (Margin of Safety) เกินกว่า 15% ขึ้นไป เพื่อลดความเสี่ยงเมื่อเผชิญแรงขาย"
        : "An undervalued stock trades below its fair value. We screen assets displaying a Margin of Safety of at least 15% under conservative Discounted Cash Flow models to act as a buffer during high market volatility."
    },
    {
      q: lang === "th" ? "ทำไม Margin of Safety ถึงมีบทบาทสำคัญสำหรับหุ้นคุณค่า?" : "Why is Margin of Safety critical for value compounders?",
      a: lang === "th"
        ? "เป็นส่วนต่างป้องกันความผิดพลาด (Error Buffer) ที่คิดค้นโดย Benjamin Graham เนื่องจากกระแสเงินสดอิสระในอนาคตอาจมีความไม่แน่นอนสูง การซื้อของราคาถูกกว่ามูลค่าเหมาะสมจะช่วยป้องกันพอร์ตจากการสูญเสียเงินต้นและเพิ่มแต้มต่อผลตอบแทนสะสม"
        : "Introduced by Benjamin Graham, it serves as an error buffer against cash flow projection mistakes. Buying assets with deep discount limits capital loss risks and maximizes capital compounding prospects."
    }
  ];

  const jsonLdFaq = {
    "@context": "https://schema.org",
    "@type": "FAQPage",
    "mainEntity": faqs.map(faq => ({
      "@type": "Question",
      "name": faq.q,
      "acceptedAnswer": {
        "@type": "Answer",
        "text": faq.a
      }
    }))
  };

  return (
    <div className="mx-auto max-w-5xl space-y-6 py-6 animate-fade-up px-4 sm:px-6">
      <script
        type="application/ld+json"
        dangerouslySetInnerHTML={{ __html: JSON.stringify(jsonLdFaq) }}
      />

      {/* BANNER */}
      <div className="surface rounded-2xl p-6 border border-line bg-surface/40 backdrop-blur-md relative overflow-hidden">
        <div className="absolute top-0 right-0 -translate-y-1/3 translate-x-1/3 w-36 h-36 bg-brand/10 rounded-full blur-2xl pointer-events-none" />
        <span className="chip border-brand/35 bg-brand/10 text-brand text-xs font-bold mb-2 inline-flex items-center gap-1">
          <Sparkles className="h-3 w-3" /> Core Keyword SEO Landing Page
        </span>
        <h1 className="font-display text-2xl sm:text-3xl font-black text-ink leading-tight">
          {lang === "th" ? titleTH : titleEN}
        </h1>
        <p className="text-xs sm:text-sm text-muted mt-1 font-semibold leading-relaxed">
          {lang === "th" ? descTH : descEN}
        </p>
      </div>

      {/* 🤖 AI ASSESSMENT CARD */}
      <Card className="border border-brand/30 bg-brand-soft/10 p-5 rounded-2xl relative overflow-hidden shadow-inner">
        <div className="absolute top-0 right-0 -translate-y-1/3 translate-x-1/3 w-36 h-36 bg-brand/10 rounded-full blur-2xl pointer-events-none" />
        <div className="flex items-center gap-2 mb-3">
          <span className="grid h-7 w-7 place-items-center rounded-lg bg-brand/20 text-brand">
            <Sparkles className="h-4 w-4" />
          </span>
          <div>
            <h3 className="text-xs sm:text-sm font-bold text-ink leading-none uppercase tracking-wider">
              🤖 {lang === "th" ? "AI วิเคราะห์จุดซื้อได้เปรียบตลาด" : "AI Undervalued Equity Assessment"}
            </h3>
            <span className="text-[9px] text-muted block mt-0.5 font-bold uppercase tracking-wider">
              Natural Language Value Synthesis • Google E-E-A-T Compliant
            </span>
          </div>
        </div>

        <p className="text-xs sm:text-sm text-ink leading-relaxed font-semibold whitespace-pre-line">
          {lang === "th"
            ? `จากการประเมินของแบบจำลอง ValuStock ปัจจุบันพบหุ้นคุณค่าที่มีราคาสมุดและส่วนเผื่อความปลอดภัยที่ยอดเยี่ยมนำโดยหลักทรัพย์เด่นอย่าง ${topThree}\n\n**บทวิเคราะห์จากปัญญาประดิษฐ์ (AI Valuation Verdict):**\nการคัดกรองหุ้นกลุ่มนี้อิงตามทฤษฎี Margin of Safety ของ Benjamin Graham เพื่อมองหาธุรกิจที่มีกระแสเงินสดเติบโตต่อเนื่องในระยะยาวแต่ราคาถูกลดทอนด้วยอารมณ์ผันผวนของตลาด ขอแนะนำกระจายพอร์ตในหลายอุตสาหกรรมเพื่อป้องกันความเสี่ยงอย่างเป็นระบบ`
            : `According to ValuStock intrinsic models, we have isolated deep value opportunities spearheaded by premium assets like ${topThree}.\n\n**AI Intrinsic Assessment:**\nThis dynamic screen utilizes Benjamin Graham's Margin of Safety principles to extract companies displaying consistent capital reserves but severely underpriced by short-term macroeconomic events. We highly advise spreading allocations across distinct sectors to insulate value portfolios from cyclical fluctuations.`}
        </p>
      </Card>

      {/* REAL-TIME SCREENED LEDGER */}
      <Card className="border border-line overflow-hidden">
        <div className="p-4 border-b border-line flex items-center justify-between">
          <div>
            <h3 className="font-display font-black text-sm text-ink">
              {lang === "th" ? "อันดับหุ้นราคาถูกกว่ามูลค่าเหมาะสมดีเยี่ยม" : "Real-time Undervalued Asset Ledger"}
            </h3>
            <p className="text-xs text-muted leading-none mt-1">
              {lang === "th" ? `อัปเดตเรียลไทม์คัดเฉพาะ 10 บริษัทแรกที่มี MOS สูงสุด` : "Sorted dynamically by highest Margin of Safety percentage"}
            </p>
          </div>
        </div>
        <div className="overflow-x-auto">
          <table className="w-full text-left text-xs border-collapse">
            <thead>
              <tr className="border-b border-line bg-elevate/60 text-muted font-bold">
                <th className="px-5 py-3.5">{lang === "th" ? "หุ้น" : "Stock"}</th>
                <th className="px-5 py-3.5 text-right">P/E Ratio</th>
                <th className="px-5 py-3.5 text-right">ROE %</th>
                <th className="px-5 py-3.5 text-right">{lang === "th" ? "ราคาเหมาะสม (Fair Value)" : "Fair Value"}</th>
                <th className="px-5 py-3.5 text-right">Margin of Safety</th>
                <th className="px-5 py-3.5 text-center">{lang === "th" ? "คำแนะนำ" : "Verdict"}</th>
              </tr>
            </thead>
            <tbody className="divide-y divide-line/60">
              {list.map(({ stock, val }: { stock: any; val: any }) => {
                const isUS = stock.currency === "USD";
                const f = isUS ? dollar(val.fairValue) : baht(val.fairValue);
                const showMos = pct(val.marginOfSafety, 0);

                return (
                  <tr key={stock.symbol} className="hover:bg-elevate/30 transition">
                    <td className="px-5 py-3 font-mono font-bold text-ink">
                      <div className="flex items-center gap-3">
                        <AssetLogo symbol={stock.symbol} color={stock.color} size="sm" />
                        <div>
                          <Link href={`/stocks/${stock.symbol.toLowerCase()}`} className="hover:text-brand underline block font-bold">
                            {stock.symbol}
                          </Link>
                          <span className="text-[10px] text-muted block truncate max-w-[120px] font-semibold">
                            {lang === "th" ? stock.name : (stock.enName || stock.name)}
                          </span>
                        </div>
                      </div>
                    </td>
                    <td className="px-5 py-3 text-right font-mono text-ink font-semibold">
                      {isFinite(val.ratios.pe) && val.ratios.pe > 0 ? `${num(val.ratios.pe, 1)}x` : "—"}
                    </td>
                    <td className="px-5 py-3 text-right font-mono text-ink font-semibold">
                      {isFinite(val.ratios.roe) ? `${num(val.ratios.roe, 1)}%` : "—"}
                    </td>
                    <td className="px-5 py-3 text-right font-mono text-gold font-bold">{f}</td>
                    <td className={`px-5 py-3 text-right font-mono font-bold ${val.marginOfSafety >= 0 ? "text-up" : "text-down"}`}>
                      {showMos}
                    </td>
                    <td className="px-5 py-3 text-center">
                      <span className={`chip border px-2 py-0.5 rounded-lg text-[10px] font-bold border-up/30 bg-up/10 text-up`}>
                        {t(`verdict.${val.verdict}`).toUpperCase()}
                      </span>
                    </td>
                  </tr>
                );
              })}
            </tbody>
          </table>
        </div>
      </Card>

      {/* FAQ */}
      <Card className="border border-line p-6 bg-surface/40 backdrop-blur-sm">
        <span className="chip border-amber-500/30 bg-amber-500/10 text-amber-400 text-xs font-bold mb-2 inline-flex items-center gap-1">
          <Info className="h-3.5 w-3.5" /> FAQ Crawlers Indexed
        </span>
        <h2 className="font-display text-xl font-black text-ink mb-5">
          {lang === "th" ? "คำถามที่พบบ่อยเกี่ยวกับหุ้นราคาถูกกว่ามูลค่าจริง" : "Frequently Asked Questions"}
        </h2>

        <div className="space-y-3.5">
          {faqs.map((faq, idx) => {
            const isOpen = openFaq === idx;
            return (
              <div
                key={idx}
                className="rounded-2xl border border-line bg-bg/40 overflow-hidden transition-all duration-300"
              >
                <button
                  onClick={() => setOpenFaq(isOpen ? null : idx)}
                  className="w-full px-5 py-4 flex items-center justify-between text-left text-xs sm:text-sm font-bold text-ink hover:bg-elevate/45 transition"
                >
                  <span>{faq.q}</span>
                  <span className="text-brand text-xs font-bold font-mono pl-3 shrink-0">
                    {isOpen ? "▲" : "▼"}
                  </span>
                </button>
                {isOpen && (
                  <div className="px-5 pb-4.5 pt-1.5 border-t border-line/25 text-xs text-muted leading-relaxed font-semibold">
                    {faq.a}
                  </div>
                )}
              </div>
            );
          })}
        </div>
      </Card>
    </div>
  );
}
